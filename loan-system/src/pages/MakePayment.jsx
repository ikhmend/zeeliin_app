...
export default function MakePayment() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!Number.isFinite(paymentAmount) || paymentAmount <= 0) return setToast({ type: "error", message: "Төлөх дүн 0-ээс их байна." });
    if (paymentAmount > totalRemaining) return setToast({ type: "error", message: "Зээлийн нийт үлдэгдлээс их дүн төлөх боломжгүй." });

    try {
      setSubmitting(true);
      await makeLoanPayment(loanId, { payment_amount: paymentAmount, payment_method: method });
      navigate(`/loans/${loanId}#payment-schedule`, {
        replace: true,
        state: { toast: { type: "success", message: "Төлөлт амжилттай бүртгэгдлээ." } },
      });
    } catch (requestError) {
      setToast({ type: "error", message: requestError.response?.data?.message || "Төлөлт бүртгэх үед алдаа гарлаа." });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div style={styles.container} className="page-container">
      ...
      <form style={styles.card} onSubmit={handleSubmit}>
        ...
        <div style={styles.notice}>
          Энэ нь зөвхөн demo төлөлт. Backend дээр DEMO_PAYMENTS_ENABLED=true үед ажиллана.
        </div>
...
const styles = {
  ...
  backButton: { marginBottom: 18, padding: 0, border: 0, background: "transparent", color: "#111827", cursor: "pointer", fontSize: 14, fontWeight: 600, textAlign: "left" },
  ...
  balance: { display: "flex", justifyContent: "space-between", gap: 16, marginBottom: 20, padding: 14, borderRadius: 8, background: "#f3f4f6", color: "#111827", fontSize: 14 },
  ...
  submit: { width: "100%", padding: "13px 16px", border: 0, borderRadius: 8, background: "#111827", color: "#fff", cursor: "pointer", fontSize: 14, fontWeight: 700, fontFamily: "inherit" },
};
