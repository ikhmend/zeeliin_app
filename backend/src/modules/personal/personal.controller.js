...
export const getProfile = asyncHandler(async (req, res) => {
  const data = await personalService.getProfileData(
    req.user.id,
...
export const updateProfile = asyncHandler(async (req, res) => {
  const data = await personalService.updateProfile(
    req.user.id,
    req.user.customer_id,
    req.validated.body,
  );
...
export const getMyLoanById = asyncHandler(async (req, res) => {
  const data = await personalService.getMyLoanById(
    req.user.customer_id,
    req.validated.params.loanId,
  );
  return Success(res, data);
});
export const getMyLoanInstallments = asyncHandler(async (req, res) => {
  const data = await personalService.getMyLoanInstallments(
    req.user.customer_id,
    req.validated.params.loanId,
  );
  return Success(res, data);
...
export const getMyLoanPayments = asyncHandler(async (req, res) => {
  const data = await personalService.getMyLoanPayments(
    req.user.customer_id,
    req.validated.params.loanId,
  );
  return Success(res, data);
...
export const makeMyPayment = asyncHandler(async (req, res) => {
  const data = await personalService.makeMyPayment(
    req.user.customer_id,
    req.validated.params.loanId,
    req.validated.body,
  );
