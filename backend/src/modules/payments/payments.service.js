...
export async function makePayment(id, paymentData) {
  const { payment_amount, payment_method, received_user_id, note } =
    paymentData;
  ...
  return await sequelize.transaction(async (transaction) => {
    ...
    const unpaidInstallments =
      await installmentsRepository.findUnpaidInstallmentsByLoanId(
        loan.id,
        transaction,
      );
    ...
    const niitUldsen =
      await installmentsRepository.getTotalRemainingAmountByLoanId(
        loan.id,
        transaction,
      );
    if (paymentCents > toCents(niitUldsen)) {
      throw new AppError(
        "Зээлийн үлдэгдлээс их дүнгээр төлөлт хийх боломжгүй.",
        400,
      );
    }
    const payments = [];
    const paidInstallments = [];
    let paymentLeft = paymentCents;
    let i = 0;
    while (i < unpaidInstallments.length && paymentLeft > 0) {
      const installment = unpaidInstallments[i];
      const remainingAmount = toCents(installment.remaining_amount);
      const payForInstallment = Math.min(remainingAmount, paymentLeft);
      const isFullyPaid = payForInstallment === remainingAmount;
      const updateData = {
        remaining_amount: fromCents(remainingAmount - payForInstallment),
        status: isFullyPaid ? "paid" : "partial",
        paid_date: isFullyPaid ? paymentDate : null,
        paid_amount: fromCents(
          toCents(installment.paid_amount || 0) + payForInstallment,
        ),
      };
      const updatedInstallment =
        await installmentsRepository.updateInstallmentPayment(
          installment.id,
          updateData,
          transaction,
        );
      const newPayment = {
        ...
        payment_amount: fromCents(payForInstallment),
        payment_date: paymentDate,
        payment_method: payment_method.trim(),
        received_user_id: received_user_id || null,
        note: note?.trim() || "",
      };
    ...
    const loanStatus =
      toCents(totalRemaining) === 0
        ? "paid"
        : (await installmentsRepository.hasOverdueInstallments(
              loan.id,
              transaction,
            ))
          ? "overdue"
          : "active";
    const updatedLoan = await loansRepository.updateLoanAfterPayment(
      loan.id,
...
export async function getPaymentsByInstallmentId(installmentId) {
  const payments =
    await paymentsRepository.findPaymentsByInstallmentId(installmentId);
  return payments.map(mapPayment);
}
